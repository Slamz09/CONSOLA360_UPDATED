  ...
  <div class="all-contracts">
    ...
    <div class="contracts-header">
      <div class="header-left">
        <h2 class="section-title">Contracts Pipeline</h2>
    ...
    <div class="table-container">
      <div class="table-header">
        ...
        <div class="col-header">Contract No</div>
		<div class="col-header">Contract Type</div>
        <div class="col-header">Counterparty</div>
		<div class="col-header">Current Stage</div>
        <div class="col-header">Value</div>
		<div class="col-header">Deal ID</div>
		<div class="col-header">Legal Approval</div>
		<div class="col-header">Business Approval</div>
		<div class="col-header">Counter Party Approval</div>
        <div class="col-header">Execution Date</div>
		<div class="col-header">Renewal Date</div>
      ...
      <div class="table-body">
        ...
        >
          ...
          <div class="col-data">
            <span class="approval-badge" :class="getApprovalClass(contract.legalApproval)">
              {{ contract.legalApproval || 'Not Required' }}
            </span>
          </div>
          <div class="col-data">
            <span class="approval-badge" :class="getApprovalClass(contract.businessApproval)">
              {{ contract.businessApproval || 'Not Required' }}
            </span>
          </div>
          <div class="col-data">
            <span class="approval-badge" :class="getApprovalClass(contract.counterpartyApproval)">
              {{ contract.counterpartyApproval || 'Pending' }}
            </span>
          </div>
          <div class="col-data date">{{ formatDate(contract.executionDate) }}</div>
          <div class="col-data date">{{ formatDate(contract.renewalDate) }}</div>
...
const contracts = ref([
  {
    ...
    businessApproval: 'Not Required',
    counterpartyApproval: 'Approved'
  },
  {
    id: 'CTR-2024-002',
    ...
    deal_id: 'DEAL-002',
    legalApproval: 'Pending Review',
    businessApproval: 'Pending Review',
    counterpartyApproval: 'Approved'
  },
  {
    id: 'CTR-2024-003',
    ...
    deal_id: 'DEAL-003',
    legalApproval: 'Approved',
    businessApproval: 'Revision Requested',
    counterpartyApproval: 'Rejected'
  },
  {
    id: 'CTR-2024-004',
    ...
    deal_id: 'DEAL-004',
    legalApproval: 'Revision Requested',
    businessApproval: 'Approved',
    counterpartyApproval: 'Pending Review'
  },
  {
    id: 'CTR-2024-005',
    name: 'Data Processing Agreement',
    counterparty: 'CloudTech Systems',
    type: 'DPA',
    value: 125000,
    status: 'Business Review',
    executionDate: '2024-03-28',
...
const filters = ref([
  ...
  { key: 'pending-approval', label: 'Pending Approval', count: 23 },
  { key: 'legal-review', label: 'Legal Review', count: 15 },
  { key: 'business-review', label: 'Business Review', count: 8 },
...
const filteredContracts = computed(() => {
  ...
  if (activeFilter.value === 'pending-approval') {
    filtered = filtered.filter(contract => 
      contract.legalApproval === 'Pending Review' ||
      contract.businessApproval === 'Pending Review' ||
      contract.counterpartyApproval === 'Pending Review'
    )
  } else if (activeFilter.value === 'legal-review') {
    filtered = filtered.filter(contract => contract.status === 'Legal Review')
  } else if (activeFilter.value === 'business-review') {
    filtered = filtered.filter(contract => contract.status === 'Business Review')
  }
...
const getStatusClass = (status) => {
  const statusMap = {
    ...
    'Legal Review': 'status-legal',
    'Business Review': 'status-business'
  }
...
const getApprovalClass = (approval) => {
  const approvalMap = {
    ...
    'Revision Requested': 'approval-revision',
    'Rejected': 'approval-rejected',
    'Not Required': 'approval-not-required'
  }
  return approvalMap[approval] || 'approval-default'
}
...
const editContract = (id) => {
  ...
  // Navigate to contract edit view
}
...
.table-header {
  display: grid;
  grid-template-columns: 40px 1.2fr 1fr 1.2fr 1fr 1fr 0.8fr 1fr 1fr 1fr 1fr 1fr 1fr 120px;
...
.type-msa { background: rgba(139, 69, 199, 0.1); color: rgb(168, 85, 247); }
.type-dpa { background: rgba(236, 72, 153, 0.1); color: rgb(244, 114, 182); }
...
.status-legal { background: rgba(99, 102, 241, 0.1); color: rgb(129, 140, 248); }
.status-business { background: rgba(139, 69, 199, 0.1); color: rgb(168, 85, 247); }
...
@media (max-width: 1600px) {
  .table-header,
  .table-row {
    grid-template-columns: 40px 1fr 0.8fr 1fr 0.8fr 0.8fr 0.6fr 0.8fr 0.8fr 0.8fr 0.8fr 0.8fr 0.8fr 100px;
  }
}
...
@media (max-width: 1200px) {
  .table-header,
  .table-row {
    grid-template-columns: 40px 1.2fr 1fr 1.2fr 1fr 0.8fr 1fr 1fr 1fr 120px;
  }
  
  .col-header:nth-child(n+11),
  .col-data:nth-child(n+11) {
    display: none;
  }
}
</style>
